...
const geminiKey = () => process.env.GEMINI_API_KEY || process.env.gemenie_API_KEY;
const apiKey = () => geminiKey() || process.env.OPENAI_API_KEY;
const model = () => geminiKey() ? (process.env.GEMINI_JUDGING_MODEL || 'gemini-3.8-flash') : (process.env.OPENAI_JUDGING_MODEL || 'gpt-4o-2024-08-06');
let evaluating = false;

export function GET() {
  return NextResponse.json({ configured: Boolean(apiKey()), model: model(), rubricVersion: RUBRIC_VERSION }, { headers: { 'Cache-Control': 'no-store' } });
}
...
export async function POST(request: NextRequest) {
  ...
  // Next normalizes loopback IPs to localhost in nextUrl; preserve the browser's actual host.
  const requestOrigin = `${request.nextUrl.protocol}//${request.headers.get('host') || request.nextUrl.host}`;
  if (request.headers.get('origin') !== requestOrigin || request.headers.get('x-judging-client') !== 'photo-workspace') return NextResponse.json({ error: 'Use the photo judging workspace to submit photographs.' }, { status: 403 });
  if (!apiKey()) return NextResponse.json({ error: 'Automatic judging needs GEMINI_API_KEY in .env.local. Add the key on the server and restart the app.' }, { status: 503 });
  ...
  try {
    ...
    const evaluation = await evaluateImage(image, apiKey()!, model());
